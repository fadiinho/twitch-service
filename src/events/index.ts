...
export function initEvents(middlewareConfig: EventSubMiddlewareConfig) {
  ...
  return {
    middleware,
    streamOnlineEvent: async (
      userName: string,
      handler?: (data: UserData, listener: EventReturnType['listener']) => void
    ): Promise<EventReturnType> => {
      const helixUser = await apiClient.users.getUserByName(userName).catch((err: Error) => {
      ...
      const listener = await middleware
        .subscribeToStreamOnlineEvents(userId, async (e) => {
          data = {
    ...
    streamOfflineEvent: async (
      user: string,
      handler?: (data: UserData, listener: EventReturnType['listener']) => void
    ): Promise<EventReturnType> => {
      const helixUser = await apiClient.users.getUserByName(user).catch((err: Error) => {
        throw new Error(err.message);
      });
      if (!helixUser) throw new Error('User not found');
      ...
      const listener = await middleware
        .subscribeToStreamOfflineEvents(userId, async (e) => {
          data = {
            broadcasterDisplayName: e.broadcasterDisplayName,
            broadcasterId: e.broadcasterId,
            broadcasterName: e.broadcasterName,
